...
- **Push Notifications (Phase 0)**: iOS native push via Firebase Cloud Messaging (FCM). `push_tokens` table stores device tokens per user. Backend `server/pushService.ts` sends via FCM Admin SDK. Frontend `registerPushNotifications()` in `capacitor.ts` requests permission and registers token on login. Endpoints: `POST /api/push/register`, `/api/push/unregister`, `/api/push/test`. Token ownership deactivates stale tokens on device re-registration. Requires `FIREBASE_PROJECT_ID`, `FIREBASE_CLIENT_EMAIL`, `FIREBASE_PRIVATE_KEY` secrets + GoogleService-Info.plist in Xcode + Push Notifications capability enabled.
