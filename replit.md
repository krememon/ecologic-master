...
- **Messaging System**: Comprehensive two-pane messaging interface for direct 1:1 conversations using a server-side "get or create then redirect" pattern. Features instant navigation, deterministic conversation creation via pairKey upsert, real-time delivery via WebSocket, read receipts, unread counts, searchable user lists, and day-based message grouping.
