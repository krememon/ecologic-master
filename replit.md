...
- **Stripe Connect (Phase 1 — Groundwork)**: Infrastructure for future subcontractor payouts. Companies table has `stripe_connect_account_id`, `stripe_connect_status`, `stripe_connect_charges_enabled`, `stripe_connect_payouts_enabled`, `stripe_connect_details_submitted`, `stripe_connect_onboarded_at` fields. `job_referrals` table stores financial snapshots on acceptance: `job_total_at_acceptance_cents`, `contractor_payout_amount_cents`, `company_share_amount_cents`. `subcontract_payout_audit` table tracks all payout attempts with idempotency keys. Service layer: `server/services/stripeConnect.ts`. Onboarding endpoints: `POST /api/stripe-connect/create-account`, `POST /api/stripe-connect/onboarding-link`, `GET /api/stripe-connect/status`, `POST /api/stripe-connect/sync`. Settings UI: `/settings/stripe-connect`. Payment detection: on Stripe payment confirmation, detects subcontracted jobs and writes preview audit records (no real transfers yet). Recommended Phase 2 architecture: **Separate Charges and Transfers**.
