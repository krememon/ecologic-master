...
- **Stripe Connect (Phase 1, 2, 3 & 4 — Complete)**: Full subcontractor payout infrastructure with **dual-transfer architecture** — EcoLogic acts as payment router, not fund holder. Companies table has `stripe_connect_account_id`, `stripe_connect_status`, `stripe_connect_charges_enabled`, `stripe_connect_payouts_enabled`, `stripe_connect_details_submitted`, `stripe_connect_onboarded_at` fields. `job_referrals` table stores financial snapshots on acceptance. `subcontract_payout_audit` table tracks dual payout attempts with `transfer_amount_cents`/`stripe_transfer_id`/`destination_account_id` for receiver AND `second_transfer_amount_cents`/`second_stripe_transfer_id`/`second_destination_account_id` for sender. Service layer: `server/services/stripeConnect.ts`. Phase 4 (Dual Transfer): On each payment, `executeSubcontractPayout()` creates TWO Stripe transfers — receiver gets `contractorPayoutCents` (the referral percentage), sender gets `companyShareCents` (the remainder). Platform retains $0. Both companies must have active Stripe Connect accounts or the payout is blocked. Pre-payment readiness check: `GET /api/stripe-connect/check-job-readiness/:jobId`. `computeSubcontractSplit()` treats `referralValue` percentage as the contractor/receiver's share. Env var `STRIPE_SUBCONTRACT_TRANSFERS_ENABLED=true`. Uses **Separate Charges and Transfers** pattern with `source_transaction`. Phase 3: Card Payment onboarding gate — `useStripeConnectGate` hook + `StripeConnectGateModal` component intercepts Card Payment flows. Readiness states: not_connected, setup_incomplete, ready, needs_attention. Owner-facing payout audit UI on Job Details page shows both transfer amounts. API: `GET /api/subcontract-payouts/job/:jobId` (Owner/Admin only).
