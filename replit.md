...
- **Contractor Network – Job Referrals** (Backend Only): Allows one contractor company to send a job to another contractor. Receiver can accept (transfers job ownership) or decline. Table: `job_referrals` with status tracking (pending/accepted/declined/completed), referral fee (percent or flat), and optional price change permission. RBAC: Owner/Admin can send/accept/decline; Supervisor can view; Technician has no access. Endpoints: `POST /api/referrals/send`, `POST /api/referrals/accept/:id`, `POST /api/referrals/decline/:id`, `GET /api/referrals/incoming`, `GET /api/referrals/outgoing`. No UI built yet.
