...
- **Payment Receipt Emails**: Automatic receipt email sent to customer after payment is recorded (all methods: cash, check, card). Triggered from: `payment_intent.succeeded` webhook, `checkout.session.completed` webhook, manual payment route, Stripe confirm endpoint, and signature capture. Centralized in `server/receiptService.ts` via `sendReceiptForPayment(paymentId)`. Idempotent via `receiptEmailSentAt` column (race-safe SQL condition). Subject: "Receipt: INV-… (Paid)" or "Receipt: INV-… (Partial)". Includes remaining balance for partial payments. PDF invoice attached if available. Tracks `receiptEmailTo`, `receiptMessageId`, `receiptError` on payments table. Debug endpoint: `POST /api/debug/payments/:paymentId/send-receipt` (owner-only). Uses Resend with branded HTML template.
...
- **Archival**: Automatic archival for estimates (when converted) and jobs (when fully paid). Centralized `recomputeJobPaymentAndMaybeArchive()` in `server/invoiceRecompute.ts` checks ALL invoices for a job before archiving. Called from: webhook handlers (`payment_intent.succeeded`, `checkout.session.completed`), manual payment routes, and Stripe confirm endpoint. Jobs use `status='archived'` + `archivedAt` + `archivedReason`. UI: "Archived" toggle on Jobs page (Owner/Supervisor), purple archived pill with date, "Restore" button to unarchive. API: `PATCH /api/jobs/:id/unarchive` (Owner/Supervisor).
...
- **Stripe Payment Element (In-App Payments)**: Card payments use Stripe PaymentIntents with `@stripe/react-stripe-js` PaymentElement rendered inline — no redirect to checkout.stripe.com. `POST /api/payments/stripe/create-intent` (authenticated) and `POST /api/public/invoices/create-intent` (public) create PaymentIntents and pre-insert a `payments` row with `status='processing'`. `StripePaymentForm` component wraps Elements provider with `redirect: 'if_required'`. Webhook `payment_intent.succeeded` upgrades the row to `status='succeeded'`, recomputes invoice totals, updates job status, sends notifications, and triggers QBO sync. Confirm endpoint (`GET /api/payments/stripe/confirm`, requires `payment_intent_id` param) also upgrades `processing→succeeded` if Stripe confirms the PI. Works in web browser and iOS Capacitor wrapper. AASA file at `/.well-known/apple-app-site-association` serves Universal Links for `/auth/*` paths only. Payment link sharing copies the public invoice URL (`/invoice/{id}/pay`) to clipboard. **No saved cards/Link**: PaymentIntents are created without `customer`, `customer_email`, or `setup_future_usage`; PaymentElement disables wallets and forces `paymentMethodOrder: ['card']`. Partial payments supported. **Old Checkout Session redirect flow fully removed**. Legacy `checkout.session.completed` webhook handler kept for any in-flight sessions.
- **Stripe Payment Status Convention**: Stripe payments use `status='succeeded'`. Manual payments (cash/check) use `status='paid'`. SUM-based recompute filter: `LOWER(status) IN ('paid', 'succeeded', 'completed')`.
