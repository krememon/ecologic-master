...
- **iPhone Native IAP Frontend (Phase 2B)**: `OnboardingSubscription.tsx` and `Paywall.tsx` detect native iOS via `isNativeIos()` and branch into the Apple IAP flow. Products loaded via `loadAppleProducts()` showing real App Store pricing. Subscribe triggers `purchaseAppleSubscription()` → JWS → `POST /api/subscriptions/validate` → billing refresh → app entry. Restore triggers `restoreApplePurchases()` → JWS → same validation path. Web/Stripe flow is completely unchanged. Library: `@capgo/native-purchases`.
