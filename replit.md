...
- **Stripe Connect (Phase 1 & 2 — Complete)**: Full subcontractor payout infrastructure. Companies table has `stripe_connect_account_id`, `stripe_connect_status`, `stripe_connect_charges_enabled`, `stripe_connect_payouts_enabled`, `stripe_connect_details_submitted`, `stripe_connect_onboarded_at` fields. `job_referrals` table stores financial snapshots on acceptance: `job_total_at_acceptance_cents`, `contractor_payout_amount_cents`, `company_share_amount_cents`. `subcontract_payout_audit` table tracks all payout attempts with idempotency keys. Service layer: `server/services/stripeConnect.ts`. Onboarding endpoints: `POST /api/stripe-connect/create-account`, `POST /api/stripe-connect/onboarding-link`, `GET /api/stripe-connect/status`, `POST /api/stripe-connect/sync`. Settings UI: `/settings/stripe-connect`. Phase 2: Real Stripe transfers via `executeSubcontractPayout()` wired into all 4 payment paths (`checkout.session.completed`, `payment_intent.succeeded` upgrade, `payment_intent.succeeded` new, and `stripe-confirm`). Env var `STRIPE_SUBCONTRACT_TRANSFERS_ENABLED=true` (set in shared env). Uses **Separate Charges and Transfers** pattern with `source_transaction` linking each transfer to its originating charge. All webhook paths resolve the charge ID and pass it to the transfer. Company 450 ("HL Pluming") linked to connected account `acct_1T8UPyPVfiifLJm9`. Owner-facing payout audit UI on Job Details page showing subcontractor fee, payout/share amounts, and transfer history with status badges. API: `GET /api/subcontract-payouts/job/:jobId` (Owner/Admin only).
