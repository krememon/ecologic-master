...
- **Stripe Connect (Phase 1, 2 & 3 — Complete)**: Full subcontractor payout infrastructure. Companies table has `stripe_connect_account_id`, `stripe_connect_status`, `stripe_connect_charges_enabled`, `stripe_connect_payouts_enabled`, `stripe_connect_details_submitted`, `stripe_connect_onboarded_at` fields. `job_referrals` table stores financial snapshots on acceptance: `job_total_at_acceptance_cents`, `contractor_payout_amount_cents`, `company_share_amount_cents`. `subcontract_payout_audit` table tracks all payout attempts with idempotency keys. Service layer: `server/services/stripeConnect.ts`. Onboarding endpoints: `POST /api/stripe-connect/create-account`, `POST /api/stripe-connect/onboarding-link`, `GET /api/stripe-connect/status`, `POST /api/stripe-connect/sync`, `POST /api/stripe-connect/ensure-ready` (creates account + onboarding link in one call if needed). Settings UI: `/settings/stripe-connect`. Phase 2: Real Stripe transfers via `executeSubcontractPayout()` wired into all 4 payment paths. Supports bi-directional payouts: subcontract-to-receiver (owner sends job out) AND referral-fee-to-sender (owner receives referred job). Env var `STRIPE_SUBCONTRACT_TRANSFERS_ENABLED=true`. Uses **Separate Charges and Transfers** pattern with `source_transaction`. Phase 3: Card Payment onboarding gate — `useStripeConnectGate` hook + `StripeConnectGateModal` component intercepts Card Payment flows in `PaymentReview.tsx` and `InvoiceDetails.tsx`. If company not Stripe-connected, owners see branded modal to start onboarding; non-owners see "owner only" message. Return flow auto-syncs status via `?stripe_connect_return=` param. Readiness states: not_connected, setup_incomplete, ready, needs_attention. Owner-facing payout audit UI on Job Details page. API: `GET /api/subcontract-payouts/job/:jobId` (Owner/Admin only).
...
- **Contractor Network – Job Referrals**: Full contractor-to-contractor job referral system with 3-tab UI (Contractors/Incoming/Sent). Table: `job_referrals` with status tracking (pending/accepted/declined/completed), referral fee (percent or flat), optional price change permission, and invite fields (`invite_token`, `invite_sent_via` (sms/email), `invite_sent_to`, `invite_sent_at`, `invite_expires_at`). RBAC: Owner/Admin can send/accept/decline; Supervisor can view; Technician has no access. Endpoints: `POST /api/referrals/send` (generates invite token, returns inviteUrl for native share sheet), `POST /api/referrals/accept/:id`, `POST /api/referrals/decline/:id`, `GET /api/referrals/incoming`, `GET /api/referrals/outgoing`. Token-based invite endpoints: `GET /api/referrals/invite/:token`, `POST /api/referrals/invite/:token/accept`, `POST /api/referrals/invite/:token/decline`. Frontend: `JobOfferInvite.tsx` at `/referrals/invite/:token`. Send Job modal uses "Share Invite" button that opens native share sheet (navigator.share) with clipboard fallback.
- **Job Offer Deep Links**: Universal Links (iOS) and App Links (Android) for `/invite/referral/*` and `/job-offer/*` paths. AASA file includes both. Primary deep link format: `/job-offer/:jobId/:token` — public GET endpoint returns job data without auth; accept/decline require auth. Fallback landing pages at both `/invite/referral/:token` and `/job-offer/:jobId/:token` show "Get EcoLogic" with App Store/Play Store buttons. Capacitor deep link handler routes both URL patterns to the correct in-app screens. `assetlinks.json` served at `/.well-known/assetlinks.json`.
