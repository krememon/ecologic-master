...
- **Stripe Native Return (Browser Close-on-Return)**: Stripe checkout returns to `/stripe/return` on the current domain. On native (Capacitor), the `/stripe/return` page detects it's inside the in-app browser, stores result/invoiceId/sessionId in `localStorage`, and calls `Browser.close()` with retry. The main app's `useStripeReturnResume` hook (App.tsx) reads the flags on mount and on `resume` event, invalidates caches, and navigates to the payment result page. Works on dev domains (replit.app) without Universal Links. AASA file at `/.well-known/apple-app-site-association` still served for production Universal Links (Xcode: `applinks:app.ecologicc.com`). The `appUrlOpen` handler also catches universal links and legacy `ecologic://stripe-return` deep links as fallbacks. The `openStripeBrowser` function in `capacitor.ts` registers a `browserFinished` listener to handle cache invalidation when the user taps Done. The `/stripe/return` page shows a fallback "Open EcoLogic" deep link button after 1.5s for dev environments where Universal Links aren't configured.
