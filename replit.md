...
- **Time Tracking (Clock In/Out)**: A "Time Today" section on the homepage for labor visibility and job-aware time tracking. Technicians can clock in/out to specific jobs or non-job categories (Shop, Drive, Admin, Break), see their hours logged today, current job/activity, and have a visual active indicator when clocked in. Features include: job picker sheet with search, Switch button to change jobs while clocked in (auto-splits time entries), and elapsed time display. Owner/Supervisor/Dispatcher see aggregate "Labor Today" data including total hours logged and number of active techs. RBAC: Technicians can only clock in/out and see their own data; managers see aggregate company data only. No payroll complexity. API endpoints: GET `/api/time/today` (role-aware response with job info), POST `/api/time/clock-in` (accepts jobId and category), POST `/api/time/switch` (ends current and starts new entry), POST `/api/time/clock-out`. Stored in `time_logs` table with company_id, user_id, job_id (nullable), category (job/shop/drive/admin/break), clock_in_at, clock_out_at, date, notes fields.

- **Auto Clock-Out**: Configurable company setting to automatically close forgotten time entries at a set time (default 6:00 PM). Owner can configure the auto clock-out time at `/customize/time-tracking` using an iOS-style wheel picker. When time entries are fetched, the system checks for open entries past the auto clock-out time and closes them automatically. Closed entries show an "Auto-closed" amber tag on Timesheets. Schema includes `auto_clock_out_time` on companies table and `auto_closed`/`auto_closed_reason` on time_logs table.
