...
import { Copy, Pencil, Plus, Power, Smartphone, RefreshCw } from "lucide-react";
...
function buildAppsflyerLink(
  ...
  // Build the web/desktop fallback URL using the same origin resolution
  // as buildTrackingLink so staging / prod are never crossed.
  const webOrigin = customerOriginForTrackingLink();
  const webFallback = new URLSearchParams();
  webFallback.set("source", c.sourceType as string);
  if (code) webFallback.set("ref", code);
  const webFallbackUrl = `${webOrigin}/signup?${webFallback.toString()}`;
  ...
  // af_web_dp: AppsFlyer desktop/web fallback — desktop browsers land here
  // instead of the App Store. iPhone and Android are unaffected.
  params.set("af_web_dp", webFallbackUrl);
    ...
    navigator.clipboard
      ...
      .then(() =>
        toast({
          ...
          description: isCustom ? "Custom AppsFlyer link copied to clipboard." : "AppsFlyer OneLink copied — routes iOS, Android, and desktop automatically.",
        }),
      )
                    ...
                    <tr key={c.id} className="hover:bg-slate-50" data-testid={`campaign-row-${c.id}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          {/* ── Campaign Link (AppsFlyer OneLink) — primary action ── */}
                          {appsflyerLink ? (
                            <>
                              <Button
                                type="button"
                                ...
                                title={
                                  appsflyerLink.isCustom
                                    ? `Copy campaign link (custom AppsFlyer) — ${appsflyerLink.url}`
                                    : `Copy campaign link — iOS → App Store · Android → Google Play · Desktop → web signup · ${appsflyerLink.url}`
                                }
            ...
            {previewAppsflyerLink ? (
              <div>
                <div className="text-slate-500 mb-1">
                  Campaign link preview{previewAppsflyerLink.isCustom ? " (custom override)" : " (AppsFlyer OneLink)"}:
                </div>
                <a
