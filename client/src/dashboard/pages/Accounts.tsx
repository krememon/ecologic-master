...
import { RefreshCw, Trash2, AlertTriangle, KeyRound, Mail } from "lucide-react";
...
function AccountDetailDrawer({
  ...
  const passwordResetMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", `/api/admin/dashboard/accounts/${companyId}/send-password-reset`);
      return res.json() as Promise<{ ok: boolean; sentTo: string }>;
    },
    onSuccess: (r) => {
      toast({ title: `Password reset sent to ${r.sentTo}` });
    },
    onError: (err: any) => {
  ...
  const emailChangeMutation = useMutation({
    mutationFn: async (newEmail: string) => {
      ...
      return res.json() as Promise<{ ok: boolean; pendingEmail: string }>;
    },
    onSuccess: (r) => {
      toast({
        title: "Email change verification sent",
        description: `Verification email sent to ${r.pendingEmail}. The account email will update once the user clicks the link.`,
      });
      setShowEmailChangeModal(false);
      setPendingNewEmail("");
    },
    onError: (err: any) => {
      toast({ title: "Failed to send email change", description: err?.message, variant: "destructive" });
    },
  });
          ...
          <>
            ...
            <div className="space-y-6 mt-6">
              ...
              <section className="space-y-3">
                ...
                <p className="text-xs text-slate-400">
                  Actions taken here are logged and send emails to the account owner at{" "}
                  <span className="font-medium text-slate-600">{account.ownerEmail ?? "—"}</span>.
                </p>
                <div className="flex flex-col gap-2">
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    disabled={passwordResetMutation.isPending || !account.ownerEmail}
                    onClick={() => passwordResetMutation.mutate()}
                    data-testid="account-send-password-reset"
                  >
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    disabled={!account.ownerEmail}
                    onClick={() => { setPendingNewEmail(""); setShowEmailChangeModal(true); }}
                    data-testid="account-send-email-change"
                  >
                    <Mail className="h-3.5 w-3.5 mr-2" />
                    Send email change / reset
                  </Button>
                </div>
              </section>

              {/* Email change modal */}
              <Dialog open={showEmailChangeModal} onOpenChange={(v) => { if (!v) { setShowEmailChangeModal(false); setPendingNewEmail(""); } }}>
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle>Send email change verification</DialogTitle>
                    <DialogDescription>
                      Enter the new email address for this account. A verification link will be sent there — the account email
                      only changes after the user clicks it.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="space-y-3 py-2">
                    <div>
                      <Label className="text-xs text-slate-500">Current email</Label>
                      <p className="text-sm font-mono text-slate-700 mt-0.5">{account.ownerEmail ?? "—"}</p>
                    </div>
                    <div>
                      <Label className="text-xs text-slate-500" htmlFor="pending-new-email">New email address</Label>
                      <Input
                        id="pending-new-email"
                        type="email"
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter" && pendingNewEmail && !emailChangeMutation.isPending) {
                            emailChangeMutation.mutate(pendingNewEmail);
                          }
                        }}
                      />
                    </div>
                    <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded px-2 py-1.5">
                      The account email does <strong>not</strong> change immediately. The user at the new address must click the verification link within 24 hours.
                    </p>
                  </div>
                  <DialogFooter>
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => { setShowEmailChangeModal(false); setPendingNewEmail(""); }}
                    >
                      Cancel
                    </Button>
                    <Button
                      type="button"
                      disabled={!pendingNewEmail.trim() || emailChangeMutation.isPending}
                      onClick={() => emailChangeMutation.mutate(pendingNewEmail.trim())}
