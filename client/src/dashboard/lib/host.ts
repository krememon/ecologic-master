 ...
 *
 * Allowed: localhost and Replit dev/deploy preview hosts. On those we have no
 * choice but to allow overrides because there is no real `dashboard.*`
 * subdomain to visit.
 *
...
function detectFromHostname(): AppMode {
  const host = (typeof window !== "undefined" && window.location?.hostname) || "";
  if (host === "dashboard.ecologicc.com" || host === "staging-dashboard.ecologicc.com") {
    return "dashboard";
  }
  return "customer";
}
...
export function getAppMode(): AppMode {
  ...
  const host = (typeof window !== "undefined" && window.location?.hostname) || "";
  if (/\.replit\.dev$/i.test(host) || /\.replit\.app$/i.test(host) || /\.repl\.co$/i.test(host)) {
    // Actively clear any stale "dashboard" override from localStorage so a
    // previous ?app=dashboard testing session can never re-activate after the
    // user navigates back to the Replit preview without the query param.
