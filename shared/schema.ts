...
export const leads = pgTable("leads", {
  ...
  convertedToJobId: integer("converted_to_job_id").references(() => jobs.id, { onDelete: "set null" }),
...
// Eco-AI Conversations
export const ecoAiConversations = pgTable("eco_ai_conversations", {
  id: serial("id").primaryKey(),
  companyId: integer("company_id").notNull().references(() => companies.id),
  createdById: varchar("created_by_id").notNull().references(() => users.id),
  title: varchar("title", { length: 255 }),
...
// Eco-AI Messages
export const ecoAiMessages = pgTable("eco_ai_messages", {
  id: serial("id").primaryKey(),
  conversationId: integer("conversation_id").notNull().references(() => ecoAiConversations.id, { onDelete: "cascade" }),
  role: varchar("role", { length: 20 }).notNull(),
  content: text("content").notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => [
  index("eco_ai_messages_conv_idx").on(table.conversationId, table.createdAt),
]);

// Eco-AI Proposed Actions
export const ecoAiActionStatusEnum = pgEnum("eco_ai_action_status", ["proposed", "confirmed", "rejected", "executed", "failed"]);

export const ecoAiActions = pgTable("eco_ai_actions", {
  id: serial("id").primaryKey(),
  conversationId: integer("conversation_id").notNull().references(() => ecoAiConversations.id, { onDelete: "cascade" }),
  messageId: integer("message_id").references(() => ecoAiMessages.id),
  tool: varchar("tool", { length: 50 }).notNull(),
  payload: jsonb("payload").notNull(),
  status: ecoAiActionStatusEnum("status").notNull().default("proposed"),
...
export const insertEcoAiConversationSchema = createInsertSchema(ecoAiConversations).omit({ id: true, createdAt: true });
export const insertEcoAiMessageSchema = createInsertSchema(ecoAiMessages).omit({ id: true, createdAt: true });
export const insertEcoAiActionSchema = createInsertSchema(ecoAiActions).omit({ id: true, createdAt: true, executedAt: true });

export type EcoAiConversation = typeof ecoAiConversations.$inferSelect;
export type InsertEcoAiConversation = z.infer<typeof insertEcoAiConversationSchema>;
export type EcoAiMessage = typeof ecoAiMessages.$inferSelect;
export type InsertEcoAiMessage = z.infer<typeof insertEcoAiMessageSchema>;
export type EcoAiAction = typeof ecoAiActions.$inferSelect;
export type InsertEcoAiAction = z.infer<typeof insertEcoAiActionSchema>;
